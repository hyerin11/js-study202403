...
// //방법1) reduce 사용했을 때 - 집계할 때 많이 사용한다.
...
//방법2) forIn 사용했을 때 - 객체니까 forEach룰 사용할 수 없음
...
//조금 더 간편하게 써본다면!------
const trsCountByCity = traders.reduce((acc, { trader }) => { //trs뿌셔줌(생략)
  const { city } = trader; //trader뿌셔줌(생략)
  if (!acc[city]) {
    acc[city] = 1;
  } else {
    acc[city]++;
